...
class ResNet(MetaModule):
    def __init__(self, args, blocks, avg_pool=True, dropblock_size=5,
                 wh_size=1, inductive_bn=False):
        self.inplanes = 3
        ...
        self.layer1 = self._make_layer(blocks[0], 64, stride=2, drop_rate=args.drop_out, drop_block=True,
                                       block_size=dropblock_size)
        self.layer2 = self._make_layer(blocks[1], 128, stride=2, drop_rate=args.drop_out, drop_block=True,
                                       block_size=dropblock_size)
        self.layer3 = self._make_layer(blocks[2], 256, stride=2, drop_rate=args.drop_out, drop_block=True,
                                       block_size=dropblock_size)
        self.layer4 = self._make_layer(blocks[3], 512, stride=2, drop_rate=args.drop_out, drop_block=True,
                                       block_size=dropblock_size)
        ...
        self.classifier = MetaLinear(512 * wh_size * wh_size, args.num_ways)
    ...
    def forward(self, x, params=None):
        ...
        if self.boil:
            ...
            params_classifier = get_subdict(params, 'classifier')
        logits = self.classifier(features, params=params_classifier)

        return logits


def ResNet12(args, inductive_bn=False):
    blocks = [BasicBlock, BasicBlock, BasicBlock, BasicBlock]
    return ResNet(args, blocks, inductive_bn=inductive_bn)
